...
trading_post_layout = [[stone,wood,wild],
                       [stone,wood,brick]]

bank_layout = [[wheat,wheat,wild],
               [wood,glass,brick]]
...
tailor_layout = [[wild,wheat,wild],
                 [stone,glass,stone]]

market_layout = [[wild,wood,wild],
                 [stone,glass,stone]]

bakery_layout = [[wild,wheat,wild],
                 [brick,glass,brick]]
...
# MONUMENTS
architects_guild_layout = [[wild,wild,glass],
                           [wild,wheat,stone],
                           [wood,brick,wild]]
...
barrett_castle_layout = [[wheat,wild,wild,stone],
                         [wood,glass,glass,brick]]

cathedral_of_caterina_layout = [[wild,wheat],
                                [stone,glass]]

fort_ironweed_layout = [[wheat,wild,brick],
                        [stone,wood,stone]]

grand_mausoleum_of_the_rodina_layout = [[wheat,wheat],
                                        [brick,stone]]

grove_university_layout = [[wild,brick,wild],
                           [stone,glass,stone]]

mandras_palace_layout = [[wheat,glass],
                         [brick,wood]]

obelisk_of_the_crescent_layout = [[wheat,wild,wild],
                                  [brick,glass,brick]]

opaleyes_watch_layout = [[wood,wild,wild,wild],
...
silva_forum_layout = [[wild,wild,wheat,wild],
                      [brick,brick,stone,wood]]

the_sky_baths_layout = [[wild,wheat,wild],
                        [stone,glass,wood],
                        [brick,wild,brick]]
        ...
        for i in range(board_rows - variant_rows + 1):
            for j in range(board_cols - variant_cols + 1):
                ...
                for r, c in not_wilds:
                    ...
                    if board_value != layout_value.get_name() and board_value != wild:      # CHECK FOR RESOURCE MISMATCH AND WILD RESOURCE
                        match = False
                        break
                if match:
                    coord_set = []
