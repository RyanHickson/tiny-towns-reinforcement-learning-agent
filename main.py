...
class Game:
    def __init__(self):
        ...
        manual_card_selection = handle_input(manual_card_selection_text, range(2), parse=int)
        ...
        if not manual_card_selection:   # Normal play, random card choices from each deck
        ...
        elif manual_card_selection:     # Allows for manual card choices for a game
            card_choice_list = []
            for deck in all_decks:
                ...
                card_choice_index = handle_input(choose_card_text.format(card_deck_dict), card_deck_dict, parse=int)
        ...
        if not manual_card_selection:
            for player in range(self.number_of_players):
                self.dictionary_of_players[player] = Player(player + 1, rdm.choice(monuments_deck), self.dictionary_of_agents[player])
            ...
            for player in range(self.number_of_players):
                ...
                monument_index = handle_input(monument_selection_text.format((player + 1), dict_enum(monument_names_deck)), range_len(monument_names_deck), parse=int)
    ...
    def play(self):
        ...
        while not finished:
            ...
            while building_choice != "FIN":  # MAIN TURN LOOP
                first_player = self.player_queue[0]   # player one becomes first player to act
                ...
                if isinstance(resource_dict[resource_choice_id], Resource):
                    for each_player in self.dictionary_of_players:    # RESOURCE PLACEMENT ROUND
                    ...
                    for each_player in self.dictionary_of_players:    # BUILDING ROUND
                        acting_player = self.dictionary_of_players[each_player]
